...
def init_student_model(lm_config):
    ...
    Logger(f'学生模型(LLM)总参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.3f}M')
...
def init_teacher_model(lm_config):
    ...
    state_dict = torch.load(ckp, map_location=args.device)
    model.load_state_dict(state_dict, strict=False)
    Logger(f'教师模型(LLM)总参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.3f}M')
    model = model.to(args.device)
    return model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default="dataset/sft_xxx.jsonl")

    args = parser.parse_args()
    # 定义学生模型和教师模型
    lm_config_student = TOMConfig(hidden_size=512, num_hidden_layers=8)
    lm_config_teacher = TOMConfig(hidden_size=768, num_hidden_layers=16)
    ...
    args.wandb_run_name = f"TOM-Dist-SFT-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
